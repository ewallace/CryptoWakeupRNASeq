...
MTT_df <- 
  tibble(MTT = as_factor(MTT_levels),
                 x = as.numeric(MTT)) %>%
  separate(MTT, c("Medium","Temperature","Time"), remove = FALSE) %>% 
  mutate(Phase = Time %>%
...
MTT_MT_df <- MTT_df %>%
  group_by(Medium,Temperature,Phase) %>%
  summarise(xmean = mean(x), xmin = min(x), xmax = max(x)) %>%
  mutate(Medium = str_replace(Medium, "RPMIS", "RPMI+"),
         Temperature = str_replace(Temperature, "C", "°C"),)

plot_nice_MTT_labels <- 
  ggplot(data = MTT_df) +
  geom_text(aes(x = x, label = Time), y = 3) + 
  geom_text(data = MTT_MT_df, aes(x = xmean, label = Medium), y = 2) + 
  geom_segment(data = MTT_MT_df,
               aes(x = xmin, xend = xmax), y = 2.5, yend = 2.5) +
  geom_text(data = MTT_MT_df, aes(x = xmean, label = Temperature), y = 1) + 
  scale_y_continuous(limits = c(0.5,3.5),
                     breaks = c(1,2,3),
                     labels = c("Temperature", "Medium", "Time (mins)")) + 
...
plot_nice_MTT_yaxis <- 
  ggplot(data = tibble(y = c(1,2,3),
                     label = c("Temperature", "Medium", "Time (mins)")),
         aes(y=y, label = label)) +
