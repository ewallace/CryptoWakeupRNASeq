...
## Facet plot of example genes

```{r plot_log2FC_line_faceted, fig.height = 6, fig.width = 3}
repcluster_genes <- c("CNAG_03012", "CNAG_06443", "CNAG_01551", "CNAG_06917", "CNAG_06125")

repcluster_genenames <- c("CQS1", "SSA1", "GAT201", "TSA3", "TEF1")

repcluster_genes_df <- 
  tibble(gene  = repcluster_genes %>% as_factor(), 
         name = repcluster_genenames %>% as_factor()) %>%
  left_join(deseq_df_MTT) %>%
    dplyr::filter(MediumTempTime != "YPD_30C_180") %>%
    dplyr::select(MTT = MediumTempTime, gene, name, log2FC) %>%
    dplyr::left_join(MTT_df, by = "MTT")

ggplot(data = repcluster_genes_df, 
       aes(x = Time, y = log2FC,
           colour = MediumTemp, group = MediumTemp)) +
  geom_hline(yintercept = 0, size = 0.5, colour = "grey90") +
  geom_line(size = 1) +
